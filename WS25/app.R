...
master_data <- patient_demo %>%
  mutate(
    ...
    # This is already done in the pre-processing script but retained in case inputted data came from another cleaning process
...
# Generate Benchmark Data for Funnel Plot (Simulated data to make the funnel plot assuming data from other hospitals in Kenya)
generate_benchmark_data <- function(knh_data) {
  ...
  base_rate <- 5 # assumes an average of 5% revision rate
  rates <- base_rate + rnorm(n_hospitals, 0, 2) - (log(volumes) * 0.5)
  rates <- pmax(0.5, rates) # Minimum expected revision rate of 0.5%

  bench_data <- data.frame(
    hospital = paste("Hospital", 1:n_hospitals), # Hospital names
    volume = volumes, # Simulated volumes
    revision_rate = rates, # Simulated revision rates
    is_knh = FALSE # Not KNH
  )
...
ui <- dashboardPage(
  skin = "black",
  dashboardHeader(
    title = "Kenyatta National Hospital Arthroplasty Registry",
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(
        ...
        fluidRow(
          box(
            width = 12, title = "Case Concentration Score (CCS) & Trends", status = "info", solidHeader = TRUE,
      ...
      tabItem(
        ...
        fluidRow(
          box(
            width = 12, title = "Volume Trends", status = "primary",
            div(
        ...
        fluidRow(
          box(
            width = 12, title = "Top Diagnoses (Treemap)", status = "primary",
      ...
      # --- Tab 3: Complications ----------
      tabItem(
        tabName = "complications",
        fluidRow(
          box(
            width = 12, title = "Surgical Complications Survival Analysis", status = "danger",
        ...
        fluidRow(
          box(
            width = 6, title = "Early Revision Reasons (<90 Days)", status = "warning",
            plotlyOutput("early_revision_reasons", height = "400px")
          ),
          box(
            width = 6, title = "Complication Risk by ASA Score", status = "warning",
...
server <- function(input, output, session) {
  ...
  themes <- list(
    "Standard" = list(primary = "#49a4ef", secondary = "lightblue", accent = "#d0712d", text = "black", bg = "white"),
  ...
  output$funnel_plot <- renderPlotly({
    ...
    p <- ggplot(bench, aes(x = volume, y = revision_rate)) +
      ...
      labs(x = "Procedure Volume", y = "Revision Rate (%)") +
      theme_minimal() +
      theme(
  ...
  output$ccs_gauge_plot <- renderPlotly({
    ...
    ) %>% layout(margin = list(l = 20, r = 20, t = 40, b = 20), paper_bgcolor = theme$bg)
  })
  ...
  output$vol_total_box <- renderValueBox({
    safe_value_box(nrow(vol_tab_data()), "Total Procedures", icon("list"), "aqua")
  })
  output$vol_primary_box <- renderValueBox({
    safe_value_box(sum(vol_tab_data()$procedure_stage == "Primary"), "Primary", icon("plus"), "green")
  })
  output$vol_rev_box <- renderValueBox({
    safe_value_box(sum(vol_tab_data()$procedure_stage == "Revision"), "Revisions", icon("redo"), "red")
  })
  ...
  output$volume_trend_plot <- renderPlotly({
    data <- vol_tab_data()
    theme <- current_theme()
    t_unit <- switch(input$vol_time_filter,
      "Week" = "week",
      "Month" = "month",
      "Year" = "year",
      "All Time" = "all"
    ...
    if (t_unit == "all") {
      plot_data <- data %>%
        summarise(n = n()) %>%
        mutate(time = "All Time")
      p <- ggplot(plot_data, aes(x = time, y = n)) +
        geom_col(fill = theme$primary) +
        labs(x = "", y = "Cases") +
        theme_minimal()
    } else {
      plot_data <- data %>%
      ...
      p <- ggplot(plot_data, aes(x = time, y = n)) +
        geom_line(color = theme$primary) +
        geom_point(color = theme$primary) +
        labs(x = "Date", y = "Cases") +
        theme_minimal()
    }
  ...
  output$top_diagnoses_plot <- renderPlotly({
    ...
    p <- plot_ly(data,
      labels = ~diagnosis_wrapped, parents = NA, values = ~n, type = "treemap",
  ...
  observeEvent(event_data("plotly_click", source = "treemap_source"), {
    ...
    if (is.null(diag_name) || is.na(diag_name) || diag_name == "") {
      return()
    }
  ...
  output$complication_risk_plot <- renderPlotly({
    ...
    theme <- current_theme()
    t_unit <- switch(input$comp_time_filter,
      "Week" = "week",
      "Month" = "month",
      "Year" = "year"
    )
